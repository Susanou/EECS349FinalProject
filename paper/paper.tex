...
\acmConference[EECS349 F19]{CWRU EECS349 '19: Final Project}{December 13, 2019}{Cleveland, OH}
...
\ccsdesc[500]{Information systems~Surfacing}
\ccsdesc[500]{Applied computing~Evidence collection, storage and analysis}
...
claim victory. Initially, the players compete in a strikingly large play area.
But as time elapses in the match, the size of the arena shrinks gradually. As a
result, players will more easily combat each other in the ultimate goal of 
surviving all other players. Fortnite is unique from video games like "Call of 
Duty"  how it handles player progression. In the "Call of Duty" franchise of
video games, as players defeat opponents, they earn "experience" which unlocks "levels" of
progression. These levels unlock new weapons and abilities to enable the player 
to intesify their customization to match the player's interests and desires. By
comparison, at the beginning of a Fortnite match, all players start with the
same, mediocre weapon and are tasked with exploring the gameplay arena for 
improved equipment. Therefore, it is impossible for players to purchase an
advantage that affects their gameplay, which raises the question of how 
Fortnite generates revenue for Epic Games. Fortnite is a free-to-play video game, which means that there is
no initial upfront cost to downloading and playing the game; however, Epic Games
has monetized the game through the availablity of a store for cosmetic items. 
These cosmetic items can only be acquired by purchasing "V-Bucks" from Epic Games.
Cosmetic items - as the name might imply - modify the external appearance of the 
player's avatar in a Fortnite match. Cosmetics are sometimes exclusive to 
certain accomplishments and time periods. Because of this, an account with 
a large collection of cosmetic items can easily have a real-world currency value
associated with it. And for exclusive items, the rarity and inability to 
earn these items as a later player means that these accounts are valuable. The 
combination of exciting, fast-paced matches and unique ways to customize one's
character avatar has helped to make Fortnite one of the most popular video games
ever. 
...
the Hyptertext Markup Language (HTML) present on the page. Unfortnuately, 
scraping the data is not as simple as traversing the pages and downloadsing the 
web page's HTML files and manullay inspecting them. Due to the large amount of 
threads and comments that must be analyzed, a program needs to be created to 
find the comment texts of each page and aggregate the comments in one file. 
And since data aggregation can be automated, the traversal of the website 
should be too. In most cases where a functional Application Programming Interface
(API) is not available, a web scraping program has to analyze the hyperlinks
available on the page to identify further pages to traverse and collect data.
Since forum style websites are neatly categorized for ease of use by visitors, 
traversal is not complicated.
...
The distribution of the Sentiment Analysis chart shows that while neutral comments may take up a plurality of the comments, no sentiment
is able to take up the majority of the overall comments. As for the positive or negative comments, while there might be more negative,
the distribution is promising considering the data was taken from a hack forum. For the trade chart it actual gives minimal
information. through manual thread analysis, we saw that the most common way of initiating a trade is through private messaging so while
the number of trades does not seem like a lot, many trades could have been done outside of the thread. Last but not least is the Q\&A
graph. This graph shows the general interest level of the community with regards to the different products and services offered. The 
interest rate is higher than the trade rate, by a lot, but this is still not quite representative of the data as many people utilize
private messaging for communication, though open thread questions about the product are still popular before PMing the seller.
